...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if fromPosition.x == CONTAINER_POSITION then
		player:sendCancelMessage(MESSAGE_STATUS_SMALL, "Put the construction kit on the floor first.")
	elseif not Tile(fromPosition):getHouse() then
		player:sendCancelMessage(MESSAGE_STATUS_SMALL, "You may construct this only inside a house.")
	else
		item:transform(kit)
