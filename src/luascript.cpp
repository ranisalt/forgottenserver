...
void ScriptEnvironment::resetEnv()
{
	...
	while (it != pair.second) {
		Item* item = it->second;
		if (item->getParent() == VirtualCylinder::get()) {
...
void LuaScriptInterface::pushCylinder(lua_State* L, Cylinder* cylinder)
{
	...
	} else if (cylinder == VirtualCylinder::get()) {
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaGameCreateItem(lua_State* L)
{
	...
	if (lua_gettop(L) >= 3) {
		...
		item->setParent(VirtualCylinder::get());
	}
...
int LuaScriptInterface::luaGameCreateContainer(lua_State* L)
{
	...
	if (lua_gettop(L) >= 3) {
		...
		container->setParent(VirtualCylinder::get());
	}
...
int LuaScriptInterface::luaItemClone(lua_State* L)
{
	...
	clone->setParent(VirtualCylinder::get());
...
int LuaScriptInterface::luaItemSplit(lua_State* L)
{
	...
	*itemPtr = newItem;
	...
	splitItem->setParent(VirtualCylinder::get());
