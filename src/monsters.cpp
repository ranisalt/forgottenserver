...
bool Monsters::deserializeSpell(const pugi::xml_node& node, spellBlock_t& sb, const std::string& description)
{
	...
	if (isScripted) {
		...
		auto combatSpellPtr = std::make_unique<CombatSpell>(nullptr, needTarget, needDirection);
...
bool Monsters::deserializeSpell(MonsterSpell* spell, spellBlock_t& sb, const std::string& description)
{
	...
	if (spell->isScripted) {
		auto combatSpellPtr = std::make_unique<CombatSpell>(nullptr, spell->needTarget, spell->needDirection);
