...
Guild* IOGuild::loadGuild(uint32_t guildId)
{
	...
	if (DBResult_ptr result = db.storeQuery("SELECT `name` FROM `guilds` WHERE `id` = {:d}", guildId)) {
		Guild* guild = new Guild(guildId, result->getString("name"));

		if ((result =
		         db.storeQuery("SELECT `id`, `name`, `level` FROM `guild_ranks` WHERE `guild_id` = {:d}", guildId))) {
			do {
...
uint32_t IOGuild::getGuildIdByName(const std::string& name)
{
	...
	DBResult_ptr result = db.storeQuery("SELECT `id` FROM `guilds` WHERE `name` = {:s}", db.escapeString(name));
	if (!result) {
		return 0;
	}
