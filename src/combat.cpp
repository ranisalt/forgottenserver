...
ReturnValue Combat::canTargetCreature(const std::shared_ptr<Player>& attacker, const std::shared_ptr<Creature>& target)
{
	...
	if (target->asPlayer()) {
		...
		if (attacker->hasSecureMode() && !Combat::isInPvpZone(attacker, target) &&
		    attacker->getSkullClient(target->asPlayer()) == SKULL_NONE) {
...
ReturnValue Combat::canDoCombat(const std::shared_ptr<Creature>& attacker, const std::shared_ptr<Creature>& target)
{
	...
	if (const auto& targetPlayer = target->asPlayer()) {
		...
		if (const auto& attackerPlayer = attacker->asPlayer()) {
			...
			// nopvp-zone
			const auto& targetPlayerTile = targetPlayer->asTile();
			if (targetPlayerTile->hasFlag(TILESTATE_NOPVPZONE)) {
				return RETURNVALUE_ACTIONNOTPERMITTEDINANOPVPZONE;
			} else if (attackerPlayer->asTile()->hasFlag(TILESTATE_NOPVPZONE) &&
		...
		if (attacker->isSummon()) {
			if (const auto& masterAttackerPlayer = attacker->getMaster()->asPlayer()) {
				...
				if (targetPlayer->asTile()->hasFlag(TILESTATE_NOPVPZONE)) {
					return RETURNVALUE_ACTIONNOTPERMITTEDINANOPVPZONE;
				}
...
void Combat::doCombat(const std::shared_ptr<Creature>& caster, const std::shared_ptr<Creature>& target) const
{
	...
	if (params.combatType != COMBAT_NONE) {
		...
		if (!params.aggressive || (caster != target && Combat::canDoCombat(caster, target) == RETURNVALUE_NOERROR)) {
			...
			combatTileEffects(spectators, caster, target->asTile(), params);
...
void MagicField::onStepInField(const std::shared_ptr<Creature>& creature)
{
	...
	if (id == ITEM_MAGICWALL_NOPVP || id == ITEM_WILDGROWTH_NOPVP) {
		if (g_game.getWorldType() == WORLD_TYPE_NO_PVP || asTile()->hasFlag(TILESTATE_NOPVPZONE)) {
			g_game.internalRemoveItem(asMagicField(), 1);
		}
		return;
	}
